...
class Admin extends MY_Controller {
       ...
       public function viewplans()
		...
		public function addpromotions()
        {
            ...
            $data = array('views'=>$views,'branches'=>$branches,"promotion"=>$promotion,"promotions"=>$promotions);//,'classes'=>$classes
			$this->load->view('admin/template/template',$data);
        } 
        ...
        public function viewfeedbacks()
        {
        	...
        	$data = array('views'=>$views, 'feedbacks'=>$feedbacks);
			$this->load->view('admin/template/template',$data);
        }
        ...
        public function viewleaves()
        {
        	...
        	$data = array('views'=>$views, 'leaves'=>$leaves);
			$this->load->view('admin/template/template',$data);
        }
        ...
        public function addenrollstudent()
        {
        	...
        	//print_r($classes);exit();
            $data = array('views'=>$views,'classes'=>$classes,'plans'=>$plans,"user_info"=>$user_info,'discount_info'=>$discount_info,'user_id'=>$id,'registration_amount'=>$user_info['registration_amount'],'active_classes'=>$active_classes);//,'classes'=>$classes
			$this->load->view('admin/template/template',$data);
        } 
				...
				*/
				...
				if(!empty($enroll_student_id))
				{
						...
						if(!empty($next_plan)){
        					...
        					$prev_invoice = $this->invoice_model->get_invoice_by_enroll_id($enroll_student_id);
        					//print_r($prev_invoice);//exit;

        					if(!empty($prev_invoice)){

						for ($i=1; $i <=$next_month_duration ; $i++) {

							$current_month = $prev_invoice['invoice_month'];
        					$current_year = $prev_invoice['invoice_year'];
        					...
        					//$cmonth = date("m", strtotime($current_month."+".$i." month"));
        					//$cyear = date("Y", strtotime($current_month."+".$i." month"));

							$enroll_students = $this->enroll_students_model->getenrollbymonth($current_month,$current_year,$enroll_student_id);

							if(!empty($enroll_students)){
					...
					$user_info = $this->users_model->getuserinfo($user_id);
                    $class_info = $this->classes_model->getclass($class_id);
					$message2["message"]	= "Dear ".$user_info['mobile'].", Received fees Rs $final_amount. Thank you, Team KAUSHIKDHWANEE";
		        	$message2["target"]	= 2;
					$message2["success"]	= 1;
					$message1 = json_encode($message2);

					$message9 = "Dear ".$user_info['name'].", Received fees Rs $final_amount. Thank you, Team KAUSHIKDHWANEE";
        ...
        public function getpricepermonthbyenroll_id()
        {
        	...
        	$student_info = $this->enroll_students_model->getenroll($enroll_id);
        ...
        public function view_democlasses()
        {
            ...
            $data = array('views'=>$views,'demos'=>$demos);
			$this->load->view('admin/template/template',$data);
        }
        ...
        public function getclassesbymemberid()
        {
        	$member_id= $this->input->post("member_id");
        	$user_info = $this->user_family_members_model->getuser($member_id);
        	$classes = $this->enroll_students_model->getuserenrolls($member_id);
        	if(!empty($classes))
        	{
		...
		public function attendance()
        {
        	...
        	$all_users = $this->users_model->getusers();
        	//$users = $this->users_model->getuserswithbranch($branch_id,$user_id,$status);
        	$users = $this->enroll_students_model->getstudentsattendance($user_id);
        	echo $this->db->last_query();exit;
