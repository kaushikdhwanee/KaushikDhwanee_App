...
class Admin extends MY_Controller {
        ...
        public function login_service()
        {
         ...
         public function dashboard()
			...
			$sumcamp_total=$this->summercamp_payment_model->collections($date);
            $invoices = $this->enroll_students_model->upcomingpendingfeessuper($date);
            $sumcamp_enroll = $this->summercamp_reg_model->getusers($date);
            $user_type= $this->session->userdata('user_type');
            ...
            $collection = $this->payments_model->todays_collection($date,$branch_id);
			 $invoice = $this->enroll_students_model->upcomingpendingfees($branch_id,$date);
             if ($user_type ==1)
             {
            $data = array('views'=>$views,'demos_count'=>count($demos),'leave_requests_count'=>count($leave_requests),"feedback_count"=>count($feedbacks),"registration_count"=>count($registrations),"enrollment_count"=>count($enrollments),"collection"=>$collections,"total"=>$total,"sumcamp_total"=>$sumcamp_total,"sumcamp_enroll"=>count($sumcamp_enroll),"upcoming_pending_fees"=>count($invoices));
            //print_r($data);exit;
             }
             else{
             	 $data = array('views'=>$views,'demos_count'=>count($demos_branch),'leave_requests_count'=>count($leave_request),"feedback_count"=>count($feedback),"registration_count"=>count($registration),"enrollment_count"=>count($enrollment),"collection"=>$collection,"total"=>$total_br,"sumcamp_total"=>$sumcamp_total,"sumcamp_enroll"=>count($sumcamp_enroll),"upcoming_pending_fees"=>count($invoice));
             }
		...
		if(!empty($_FILES['image']['name']))
				{
					...
					$config['upload_path']          = 'uploads/qr_codes/';
	               ...
	               /* $config['max_width']            = 1024;
	                ...
	                $userDetails['qrcode'] = $config['file_name'];
            	}
       ...
       $this->load->view('admin/template/template',$data);
        ...
        public function insertplan()
        {
			...
			if($this->form_validation->run())
			{
				...
				$two_session_2 = $this->input->post('two_session_2months');
				...
				$userDetails = array(
					            ...
					            'two_session_two_months'=>$two_session_2,
								'created_date'=> date("Y-m-d H:i:s")
								);
...
public function studentfees()
        {
        	...
        	//$plans = $this->plans_model->getplans(1);
             //$attendence=
			$invoice = $this->invoice_model->getinvoicebymember($user_id);
...
$this->email->subject("Account confirmation mail");
	                    ...
	                    $buffer = $this->file_download($payment_id);
        	...
        	if($this->form_validation->run())
			{
				...
				if($reg_type==1)
        			...
        			$this->email->subject("Account confirmation mail");
						...
						$userDetails = array('branch_id'=> $branch_id,
									 'message'=> "Welcome to KaushikDhwanee! Payment of Rs $registration_amount has been made successfully on date('d-m-Y',strtotime($date)) at date('h:i A',strtotime($date))",
									 'user_id'=>$user_id,
									 'created_on'=>date("Y-m-d H:i:s")
									);
						...
						$message = "Welcome $name to KaushikDhwanee. You have been successfully registered. Team KAUSHIKDHWANEE";
						// $response2 = $this->sms->sendsms($mobile,$message);
...
$users = array("enroll_student_id"=>$enroll_student_id,
                        ...
                        // echo "<script>console.log(".json_encode($student_unique_id).");</script>";
                         ...
                         $resp = $this->attendence_list_model->savedata($userdetail);
                         	...
                         	$paytimeusers= array('EmpID'=>$enroll_id,
												 ...
												 'EmpName'=>$user_enroll['name'],
												 'EmpPunchID'=>$enroll_student_id);
												 
								$this->paytime_enroll_model->save($paytimeusers);
					
					     $userd = array('enroll_status'=> 2);
                         $this->user_family_members_model->updateenrollstatus($userd, $member_id);
        ...
        public function generateinvoice_cron()
        {
        	...
        	function getculumnname($name)
        	{
        		switch ($name) {
        					...
        					case'5':
									return "two_session_one_year";
								break;
						case'6':
									return "three_session_one_year";
								break;
						case'7':
									return "one_session_three_months";
								break;
        	...
        	$invoice_end_date="";
        	$enroll_students = $this->enroll_students_model->getenrollsbyactiveusers($current_date);
			...
			$sibling_discount = 0;
				...
				$message1="Happy Holi! Team  KAUSHIKDHWANEE";
				$message ="Dear ".$name." Your fees for class ".$student_info['class_name']." is due on ".$student_info['next_fees_due_date']." Please visit your center and pay the fees at the earliest. Happy learning! Team  KAUSHIKDHWANEE";
	        	...
	        	$invoice= $this->invoice_model->getinvoicebyid($enroll_id);
	        	...
	        	$userdtails1 = array(
							...
							if(!empty($invoice)){
							$this->invoice_model->update($userdtails1,$invoice['id']);
						}			
                       else{
							array_push($invoices, $userdtails1);
	   ...
	   public function geusersbymembernclass()
	   {
        ...
        public function viewstudents($sort_by = 'enroll_student_id', $sort_order = 'DESC')
        {
			...
			$users = $this->enroll_students_model->getuserswithbranch($branch_id,$userid,$class_id,$plan,$status,$sort_by, $sort_order);
        	$views = array('viewstudents');
        	$branches = $this->branches_model->getbranches(1);
			$classes = $this->classes_model->getclasses(1);
        	$data = array('views'=>$views,'users'=>$users,"all_users"=>$all_users,"branches"=>$branches,'classes'=>$classes, 'user_id'=>$userid);
        ...
        public function updatestudentstatus()
        {
			...
			$response = $this->enroll_students_model->update($userDetails1,$id);
		...
		if($user_type==4){
		   $branch_id = $admin_info['branch_id'];
		   $users = $this->users_model->getuserbybrnch($branch_id);
		   }
        	else{
			$users = $this->users_model->getusers(1);
			}
        ...
        public function viewpendingpayments()
        {
           ...
           $user_id = $this->session->userdata('id');
			...
			$invoice = $this->invoice_model->getinvoices($branch_id, $month,1,$date);
				...
				if(!empty($payable_amount))
				{
					...
					foreach ($payable_amount as $key => $value) {
						...
						'final_amount'=>$final_amount);
							...
							$this->invoice_model->update($user,$invoice[$key]);
	...
	public function enterattendence()
        ...
        public function month_attendance()
        {
		 ...
		 {
			 $enroll_id = $this->uri->segment(3);
			 $views = array('student_progress.php');
			 $progress = $this->teachers_upload_model->teacher_uploadbyid($enroll_id);
			 $profile = $this->enroll_students_model->getenrollbyid($enroll_id);
             $data = array('views'=>$views,'progress'=>$progress,'profile'=>$profile);
			 $this->load->view('admin/template/template',$data);
		 }
...
public function summercamp()
        {
            ...
            $views = array('summercamp');
        	//$classes = $this->classes_model->getclasses(1);
        	$branches = $this->location_model->getbranches(1);
        	
        	$data = array('views'=>$views,'branches'=>$branches); //,'classes'=>$classesS
			$this->load->view('admin/template/template',$data);
		...
		$users = array(
	       ...
	       $message9 = "Dear ".$name." You have been enrolled into Summer Camp starting from $start_date. Happy learning! Team KAUSHIKDHWANEE";
					...
					if(!empty($mobile))
					{
						...
						$this->email->subject("Enrollment of Summer Camp");
	                    $this->email->from('kaushikdhwanee@gmail.com', 'Kaushik dhwanee');
...
$this->session->set_flashdata('message', $message6);
						redirect('admin/reciept/'.$response1);
				}else
				{
					$this->session->set_flashdata('message', "validation Error, Try again");
				}
	...
	public function reciept()
        {
        	$views = array('recieptpdf');
        	$payment_id = $this->uri->segment(3);
        	$payments = $this->summercamp_payment_model->getpayment($payment_id);
        	//$payment_invoice = $this->payments_invoice_model->getpayment_invoice($payment_id);
        	//echo $this->db->last_query();exit;
        	$data = array('views'=>$views,'payments'=>$payments); //,'classes'=>$classes
			$this->load->view('admin/template/template',$data);
        }
	
	public function viewcampstudents()
        {
        	
        	$branch_id =null;
        	$user_id = null;
            $branch_id = $this->input->get('branch_id');
			$all_users = $this->summercamp_reg_model->allusers();
            $userid = $this->input->get('user_id');
        	$users = $this->summercamp_reg_model->getuser($branch_id,$userid);
        	$views = array('viewcampstudents');
        	$branches = $this->location_model->getbranches(1);
        	$data = array('views'=>$views,'users'=>$users,"all_users"=>$all_users,"branches"=>$branches, 'user_id'=>$userid);
			$this->load->view('admin/template/template',$data);
		} 
	
	public function sumcamp_collection()
        {    
        	$date = date('Y-m-d');
        	$views = array('todays_camp_collection');
        	$collections = $this->summercamp_payment_model->getusers($date);
        	$branches = $this->location_model->getbranches();
           $branch_id = $this->input->get('branch_id');
            $collection = $this->summercamp_payment_model->getusersbybranch($date,$branch_id);
	...
	public function sumcamp_enroll()
        {    
        	$date = date('Y-m-d');
        	$views = array('todays_camp_enroll');
        	$collections = $this->summercamp_reg_model->getusers($date);
        	$branches = $this->location_model->getbranches();
           $branch_id = $this->input->get('branch_id');
            $collection = $this->summercamp_reg_model->getusersbybranch($date,$branch_id);
	 ...
	 public function addlocation()
        {
        	...
        	if(!empty($id))
        	{
        		$branch_info = $this->location_model->getbranch($id);
        	}

        	$views = array('addlocation');
        	$data = array('views'=>$views,'branch_info'=>$branch_info);//,"service_info"=>$service_info
			$this->load->view('admin/template/template',$data);
        }
        ...
        public function insertlocation()
        {
			...
			if($this->form_validation->run())
			{
            	...
            	{
            		//print_r($_POST);exit;
					$response = $this->location_model->update($userDetails,$id);
					
            	}else
            	{
            		$response = $this->location_model->save($userDetails);
            	}
         ...
         public function summerplan()
        {
        	...
        	$views = array('summerplan');
        	//$classes = $this->classes_model->getclasses(1);
        	$branches = $this->location_model->getbranches(1);
        	//print_r($classes);exit();
            $data = array('views'=>$views,"plan_info"=>$plan_info,'branches'=>$branches);//,"service_info"=>$service_info
			$this->load->view('admin/template/template',$data);
        }

        public function insertsummerplan()
        {
			...
			if($this->form_validation->run())
			{
				...
				$branch_id = $this->input->post('branch_id');
				$one_week = $this->input->post('one_week');
				$two_week = $this->input->post('two_week');
				$three_week = $this->input->post('three_week');
				$four_week = $this->input->post('four_week');
				$five_week = $this->input->post('five_week');
				...
				if(!empty($id))
            	{
					$response = $this->summerplan_model->update($userDetails,$id);
					
            	}else
            	{
            		$response = $this->summerplan_model->save($userDetails);
            	}
...
public function deletelocation()
        {
			...
			$response = $this->location_model->delete($id);
	...
	public function viewsummerplan()
        {
        	$plans = $this->summerplan_model->getplans();
        	
        	$views = array('viewsummerplan');
 ...
 public function extendedfee()
	 ...
	 echo $id;
        	...
        	if(!empty($id))
        	{
        		$enroll_info = $this->summercamp_reg_model->getusersbyid($id);
				echo "abc";
        	
        	}
        	$views = array('extendedfee');
        	  
        	
        	$data = array('views'=>$views,'enroll_info'=>$enroll_info);
			$this->load->view('admin/template/template',$data);
        }

public function insertextendedfee()
        {
...
if($this->form_validation->run())
		...
		$users = array(
					...
					$response = $this->summercamp_reg_model->update($users,$id);
	       ...
	       $message9 = "Dear ".$name." Your Summer Camp has been extended for $plan starting from $start_date. Happy learning! Team KAUSHIKDHWANEE";
					...
					if(!empty($mobile))
					{
                        ...
                        $message5 = "Your Summer Camp has been extended for $plan starting from $start_date. Happy learning! Team KAUSHIKDHWANEE";                                    
						$this->email->subject("Extension of Summer Camp");
	                    $this->email->from('kaushikdhwanee@gmail.com', 'Kaushik dhwanee');
	                    $buffer = $this->save_downloads($response1);
                		$this->email->attach($buffer, 'attachment', 'print_receipt.pdf', 'application/pdf');
	                    $this->email->set_newline("\r\n");
	...
	public function view_sumcamp_collection(){
      	...
      	$date= date('Y-m-d');
           ...
           // $admin_info =$this->admin_roles_model->getrole($user_id);
            //$branch_id[] = $admin_info['branch_id'];
        	$views = array('sumcamp_collections');
            ...
            $collections = $this->summercamp_payment_model->summer_collections($start_date,$end_date);
          //  $total = $this->summercamp_payments_model->tot_collections($start_date,$end_date);
           $collection = $this->summercamp_payment_model->collectionsbybranch($start_date,$end_date,$branch_id);
           // $total_br = $this->summercamp_payments_model->tot_collectionsbybranch($start_date,$end_date,$branch_id);

            if(!$branch_id){
            	$data = array('views'=>$views,'branches'=>$branches,'collection'=>$collections,'start_date'=>$start_date,'end_date'=>$end_date);
            }
            
			        
        else{
        $data = array('views'=>$views,'branches'=>$branches,'collection'=>$collection,'start_date'=>$start_date,'end_date'=>$end_date);
        }
       $this->load->view('admin/template/template',$data);
      }
	
	public function campstudentstatus()
        {
			...
			$response = $this->summercamp_reg_model->update($userDetails,$id);
			

			if($response)
			{
				$this->session->set_flashdata('message', 'Student status updated successfully..');
				redirect('admin/viewcampstudents');
			}else
			{
				redirect("admin/viewcampstudents");
			}
	...
	public function pending_fees_sms(){
	$date = date('Y-m-d');
$invoices = array();
	$users= $this->enroll_students_model->getenrollactive($date);
	foreach ($users as $key => $student_info) {
			...
			$name= $student_info['name'];
			$message ="Dear ".$name." Your fees for class ".$student_info['class_name']." is due on ".$student_info['end_date']." Please visit your center and pay the              fees at the earliest. Happy learning! Team  KAUSHIKDHWANEE";
			$this->sms->sendsms($mobile,$message);
			$userdtails1 = array(
				'enroll_student_id' => $student_info['id'],
				'amount' => $student_info['course_fee'],
				'final_amount' => $student_info['course_fee'],
...
public function upcomingpayments()
        {
           ...
           $user_id = $this->session->userdata('id');
			...
			$views = array('upcomingpendingpayments');
